...
export function RecordingOverlay({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6 pt-[max(1.5rem,env(safe-area-inset-top))] pb-[max(1.5rem,env(safe-area-inset-bottom))] backdrop-blur-sm transition-opacity duration-200"
      role="dialog"
      aria-modal="true"
      ...
      <div className="relative z-10 flex max-w-sm flex-col items-center gap-6 rounded-3xl bg-card px-8 py-10 shadow-2xl ring-1 ring-black/5 transition-transform duration-200">
        {micError ? (
          ...
          <>
            ...
            >
              <MicIcon
                className={cn('transition-transform duration-200', status === 'recording' && 'scale-110')}
              />
            </div>
